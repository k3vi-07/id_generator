...
class IdNumber(str):
    ...
    @classmethod
    def generate_id(cls, sex=0, id_number=0):
...
if __name__ == '__main__':
    opts, args = getopt.getopt(sys.argv[1:],'hn:a:',['help','num=','area='])
    num = 1
    area = 0
    for opt,arg in opts:
        if opt in ('-h','--help'):
            print("""
            身份证批量生成器，根据条件生成指定数量的身份证，默认随机生成一条记录
            -h 帮助文件
            -n 随机生成n条身份证, 默认一条
            -a 按照6位区域码， 默认随机
            """)
    ...
    print(IdNumber(IdNumber.generate_id(1, area)).get_area_name())
    idList = set([])
    while len(idList) < num:
        random_sex = random.randint(0, 1)  # 随机生成男(1)或女(0)
        id_number = IdNumber.generate_id(random_sex, area)  # 随机生成身份证号
        if IdNumber.verify_id(id_number):
            idList.add(id_number)
            print(id_number)